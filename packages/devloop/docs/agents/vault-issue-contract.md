...
- **Session note** (`/wrap`) owns **cross-issue synthesis, decisions, and
  ...
  session*. Plan-time *forecast* decisions are minted outside the loop, at
  grill time: inline by the installed vault-routed `grilling` skill as each
  fork is confirmed, and by the `/plan-distill` sweep
  ([`plan-distill.command.md`](plan-distill.command.md)) for headless grills
  or missed forks. A spec or ticket then cites those `dec-*` ids in its body
  (see `issue-tracker.md`), which is how the loop's prime step finds them. A
  separate surface at a separate moment; not a second writer racing the loop
  for this record.)
